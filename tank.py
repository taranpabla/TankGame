...
def loadResources(gvr):
	...
	#ADDED
	# for mine
	gvr.mineImages= loadImageArr("images/mine/mine",\
		0, 0, range(1,5))
		...
		if bStart:
			...
			for ch in line:
			  ...
			  if ch=="H": 
			    ...
			    obj = Mine(gvr.mineImages, x, y, gvr)
			  elif ch=="G": 
...
def collisionTest(gvr):
	...
	#ADDED
	#TANK COLLISON WITH MINE
	dict5 =pygame.sprite.groupcollide(gvr.lstMines,gvr.lstTanks,\
			False, False)
	for mine in dict5.keys():
		mKillMe = False
		for tank in dict5[mine]:
			if tank!=mine.update():
				tank.hit()
				mine.hit()
				mKillMe = True #the mine should explode too
		if mKillMe: mine.hit()
...
class globalvar:
	...
	#ADDED
	lstMines = pygame.sprite.RenderClear(()) 
